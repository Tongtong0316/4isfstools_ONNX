...
fn apply_python_subprocess_env(command: &mut Command) {
    command.env("PYTHONUTF8", "1");
    command.env("PYTHONIOENCODING", "utf-8");
    #[cfg(windows)]
    {
        if let Some(node_dir) = detect_windows_node_bin_dir() {
...
#[cfg(windows)]
fn detect_windows_node_bin_dir() -> Option<PathBuf> {
    let candidates = [
        r"C:\Program Files\nodejs",
        r"C:\Program Files (x86)\nodejs",
        r"C:\Users\suntong\AppData\Local\Programs\nodejs",
    ];
    candidates
        .iter()
        .map(PathBuf::from)
        .find(|dir| dir.join("node.exe").exists())
}
...
fn js_runtime_ready() -> bool {
    #[cfg(windows)]
    {
        detect_windows_node_bin_dir().is_some()
    }
    #[cfg(not(windows))]
    {
        true
...
fn run_python_module(app: &AppHandle, module: &str, args: &[&str]) -> Result<String, String> {
    ...
    apply_python_subprocess_env(&mut command);
...
pub async fn get_online_import_status(
    ...
    let js_runtime_ready = js_runtime_ready();
    ...
    let can_download = python_ready && ffmpeg_ready && ytdlp_ready && js_runtime_ready;
